...
Description:
    Derived class from QGraphicsItem that represents a labeled object with label associated to it.
...
class ImageLabelBox(QtWidgets.QGraphicsItem):
    def __init__(
        ...
        label: int,
        labelText: str,
        textScale_fn: Callable[[], float],
        imageRect: QRectF,
        selectedColor: QColor | None = None,
        defaultColor: QColor | None = None,
    ) -> None:
        """
        Docstring for __init__

        :param rect: PySide6 QRectF describing dimensions of
        :type rect: QRectF
        :param label: Description
        :type label: int
        :param labelText: Description
        :type labelText: str
        :param textScale_fn: Description
        :type textScale_fn: Callable[[], float]
        :param imageRect: Description
        :type imageRect: QRectF
        :param selectedColor: Description
        :type selectedColor: QColor | None
        :param defaultColor: Description
        :type defaultColor: QColor | None
        """
        super().__init__()
        ...
        self._handle_fn = textScale_fn if (textScale_fn is not None) else (lambda: 50)
    ...
    def paint(
        ...
        painter.drawRect(QRectF(-self._wHalf, -self._hHalf, self.w, self.h))
        ...
        painter.setFont(QFont("Arial", min(max(self.w * 0.1, 20), 80)))  # type: ignore
        painter.drawText(
            self.getTextRect(),
    ...
    def getTopLeftRect(self) -> QRectF:
        return QRectF(-self._wHalf, -self._hHalf, self._handleSize, self._handleSize)

    def getTopRightRect(self) -> QRectF:
        return QRectF(
            self._wHalf - self._handleSize,
            -self._hHalf,
            self._handleSize,
            self._handleSize,
        )
    ...
    def getTextRect(self) -> QRectF:
        return QRectF(
            -self._wHalf + self._handleSize, -self._hHalf, self._wHalf, self._hHalf
        )
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        if self._isBeingDragged:
            ...
            if self._topLeftMoving:
                ...
                raise Exception(
                    "Error: Mouse event for resizing was triggered not by handle rectangles!"
                )
            ...
            if (
                movePointX >= self.imageRect.x()
                and movePointX <= self.imageRect.x() + self.imageRect.width()
            ):
                tmpX = newCenterX
                self.w = newWidth
            ...
            if (
                movePointY >= self.imageRect.y()
                and movePointY <= self.imageRect.y() + self.imageRect.height()
            ):
                tmpY = newCenterY
                self.h = newHeight
    ...
    def updateColors(
        self, defaultColor: QColor | None, selectedColor: QColor | None
    ) -> None:
        self._defaultColor: QColor = (
            defaultColor
            if (defaultColor is not None and defaultColor.isValid())
            else QColor(40, 40, 40)
        )
        self._selectedColor: QColor = (
            selectedColor
            if (selectedColor is not None and selectedColor.isValid())
            else QColor(100, 100, 255)
        )

        self._penWidth: float = 5
        self._pen: QPen = QPen(
            QBrush(self._defaultColor), self._penWidth, s=Qt.PenStyle.SolidLine
        )

        self._backgroundBrush: QBrush = QBrush(
            self._defaultColor, Qt.BrushStyle.BDiagPattern
        )
        self._handleBrush: QBrush = QBrush(
            self._defaultColor, Qt.BrushStyle.SolidPattern
        )
