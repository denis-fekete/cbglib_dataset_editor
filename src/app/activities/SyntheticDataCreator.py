...
class SyntheticDataCreator(AbstractTabWidget):
    ...
    def _initTopBarContainer(self) -> None:
    ...
    def _initPresetSelectorContainer(self) -> None:
        ...
        self._presetSelectorContainer.setSizePolicy(
        ...
        self._presetSelectorContainer.layout().addWidget(
            QtWidgets.QLabel("Filters:"), 0, 0
        )
        ...
        self._presetSelectorContainer.layout().addWidget(
            self._filterSelector, 2, 0, 1, 2
        )

    def _initImageSettingsContainer(self) -> None:
        ...
        self._imageSettingsContainer.setSizePolicy(
            QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum
        )
        ...
        self.saturationSlider.valueChanged.connect(
            self.syntheticImage.saturationSliderChanged_slot
        )
        ...
        self.contrastSlider.valueChanged.connect(
            self.syntheticImage.contrastSliderChanged_slot
        )
        ...
        self.brightnessSlider.valueChanged.connect(
            self.syntheticImage.brightnessSliderChanged_slot
        )
        ...
        self.hFlipCheckbox.toggled.connect(
            self.syntheticImage.horizontalFlipCheckboxChanged_slot
        )

        self.vFlipCheckbox = QtWidgets.QCheckBox("Vertical flip")
        self.vFlipCheckbox.toggled.connect(
            self.syntheticImage.verticalFlipCheckboxChanged_slot
        )
        ...
        self.sapNoiseSlider.valueChanged.connect(
            self.syntheticImage.sapNoiseSliderChanged_slot
        )
        ...
        self.gaussianNoiseSlider.valueChanged.connect(
            self.syntheticImage.gaussianNoiseSliderChanged_slot
        )
        ...
        saturationContainer.addWidgets(
            [self._settingsSaturationText, self.saturationSlider]
        )
        ...
        brightnessContainer.addWidgets(
            [self._settingsBrightnessText, self.brightnessSlider]
        )
        ...
        gaussianNoiseContainer.addWidgets(
            [self._settingsGaussianNoiseText, self.gaussianNoiseSlider]
        )
    ...
    def _initImagePreviewContainer(self) -> None:
    ...
    def saveSyntheticImage_slot(self) -> None:
        ...
        for sFilter in SharedValues().filterPresets:
            self.syntheticImage.filter = sFilter
            self.syntheticImage.save(
                Path(os.getcwd()).resolve()._str, Path(os.getcwd()).resolve()._str
            )

    def newFilter_slot(self) -> None:
    ...
    def deleteFilter_slot(self) -> None:
        """Deletes filter from global list of filters, all keys will be moved down"""
        index = self._filterSelector.currIndex
        if index is None:
            return
        SharedValues().filterPresets.pop(index)
        self._filterSelector.loadLabels()

    def filterSelected_slot(self, index: QModelIndex) -> None:
