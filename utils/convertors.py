...
def Mat2QImage(mat: cv.Mat) -> QImage:
    h, w, ch = mat.shape
    bytesPerLine = ch * w
...
def Norm2Pixels(x: float, y: float, w:float, h:float, imgW:float, imgH:float) -> tuple[float, float, float, float]:
    """
...
def Pixels2Norm(x: float, y: float, w:float, h:float, imgW:float, imgH:float) -> tuple[float, float, float, float]:
    """
